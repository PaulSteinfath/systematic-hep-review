summarize_cfa_criteria <- function(df) {
  ...
  # Split comma-separated entries and map them
  ...
  # Subset algorithms for smaller inset plot
  algo_data <- df %>%
    separate_rows(cfa_rej_criteria, sep = ",") %>%
    mutate(
      cfa_rej_criteria = str_replace_all(cfa_rej_criteria, "[\r\n]", " "),
  ...
  # Only create inset if algorithm data exists
  if (nrow(algo_data) > 0) {
    algo_plot <- ggplot(algo_data, aes(x = cfa_rej_criteria)) +
      ...
      scale_x_discrete(labels = function(x) sapply(x, function(xi) {
        if (xi == "corrmap") "Corrmap"
        else if (xi == "iclabel") "IClabel"
        else if (xi == "sasica") "SASICA"
        else xi
      })) +
